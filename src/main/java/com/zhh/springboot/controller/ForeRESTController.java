...
import com.zhh.springboot.comparator.ProductPriceComparator;
import com.zhh.springboot.comparator.ProductReviewComparator;
import com.zhh.springboot.comparator.ProductSaleCountComparator;
import com.zhh.springboot.model.*;
import com.zhh.springboot.service.*;
...
public class ForeRESTController {
    ...
    public Object login(@RequestBody User user, HttpSession session) {
        ...
        try {
            ...
            subject.login(token);
            //没有报错，则说明 登录成功
            User byName = userService.getByUsername(name);
...
//        return Result.fail("请先登录！");
    ...
    public Object search(String keyword, HttpSession session) {
        if (null == keyword)
            keyword = "";
        PageResult pr  = productService.search(keyword, 0, 20);
        List<ProductES> ps = pr.getProductES();
        Long totalHits = pr.getTotal();
        List<Product> products = new ArrayList<>();
        System.out.println(ps);

        for(ProductES productES : ps){
            int id = productES.getId();
            Product p = productService.get(id);
            products.add(p);
            System.out.println(p.getName());
            productService.setSaleAndReviewNumber(p,productES);
            System.out.println(productES.getSaleCount());
        }
        //设置该产品的图片
        productImageService.setFirstProductESImages(ps);

        //设置该产品的销售与评价数量
//        productService.setESSaleAndReviewNumber(ps,products);
    ...
    public Object buy(String[] oiid, HttpSession session) {
        ...
        for (String strid : oiid) {
            ...
            System.out.println("vvvvvvvvvvvvvvvvvv "+id);
            //获取订单项
...
//            return Result.fail("请登录");
    ...
    public Object review(int oid) {
        ...
        for(int i=0; i<order.getOrderItems().size(); i++){
            ...
            listPro.add(product);
            map.put("reviews", list);
            System.out.println("xxxxxxxxxxxxxxx"+product);
        }
        map.put("p", listPro);
        map.put("o", order);
    ...
    @PostMapping("forecompare")
    public Object Compare(String keyword, String type, HttpSession session){
        if (null == keyword)
            keyword = "";
        if (null == type)
            type = "default";
        List<ProductES> ps = (List<ProductES>) session.getAttribute("ps");
        Long totalHits = (Long) session.getAttribute("totalHits");
